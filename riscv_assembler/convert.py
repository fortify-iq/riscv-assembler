...
class EmptyFile(Exception):
    def __init__(
        self,
        message="File either doesn't exist, has no code, or all is commented out.\nInvestigate any tab/spacing syntax issues",
    ):
        self.message = message
        super().__init__(self.message)
...
class AssemblyConverter:
    ...
    def __init__(self, output_type="b", nibble=False, filename="", hexMode=False):
        ...
        if (
            "b" not in output_type
            and "t" not in output_type
            and "p" not in output_type
            and "r" not in output_type
        ):
    ...
    def __str__(self):
        return "AssemblyConverter(output_type={}, nibble={}, filename={}, hexmode={})".format(
            self.output_type, self.nibble, self.filename, self.hexMode
        )
    ...
    def SB_type(self, instr, rs1, rs2, imm):
        ...
        # represent `imm` as 13-bit in two's complement form
        imm = format(((1 << 13) - 1) & imm, "013b")
    ...
    def UJ_type(self, instr, imm, rd):
        ...
        # represent `imm` as 21-bit in two's complement form
        imm = format(((1 << 21) - 1) & imm, "021b")
    ...
    def __interpret(self, line, i):
        # replace hex values by decimal ones
        hex_value = [
            word for word in line.split() if word.startswith("0x")
        ]  # there's just one hex per line
        if hex_value != []:
            hex_value = hex_value[0].split("(", 1)[
                0
            ]  # remove every character after "(" (e.g. in "0x400(x0)")
            line = line.replace(hex_value, str(int(hex_value, 16)))
        ...
        if clean[0] in self.R_instr:
            ...
            if len(clean) == 3:
                res.append(
                    self.UJ_type(clean[0], self.calcJump(clean[2], i), self.__reg_map(clean[1]))
                )
            else:
                res.append(
            ...
            if clean[0] == "li":  # need to consider larger than 12 bits
                ...
                res.append(
                    self.I_type("addi", self.__reg_map(clean[2]), "0", self.__reg_map(clean[1]))
                )
            elif clean[0] == "not":
                res.append(
                    self.I_type("xori", self.__reg_map(clean[2]), "-1", self.__reg_map(clean[1]))
                )
            elif clean[0] == "neg":
                res.append(
                    self.R_type(
                ...
                res.append(
                    self.I_type("jalr", self.__reg_map(clean[1]), "0", self.__reg_map("x0"))
                )
            elif clean[0] == "ret":
